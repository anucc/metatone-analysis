...
\begin{abstract}
  Interfaces for supporting collaborative creativity are often
  evaluated with user ratings and interviews. Such data can be
  impractical to collect in-the-wild, for example during musical
  performances. In this paper we show how a transition matrix based
  post-hoc analysis of logged interaction data from a long program of
...
Computing devices and interfaces continue to open up rich, open-ended
interaction contexts such as group music-making. Understanding and
evaluating interfaces in these contexts presents many challenges;
making use of the easily-logged interaction data without descending
into ``quantitative reductionism'', as well as balancing the desire to
gather ``felt experience'' data through surveys/interviews without
unnecessary interruptions to the participants.

In this paper we show how we have used post-hoc analyses of log data
to support a long-term practice-led research program into iPad
interfaces for group music-making. Over a period of more than two
years (April 2013--June 2015), we have collected data from $150$ group
music-making sessions where participants used musical touch-screen
interfaces on Apple iPad devices. These interfaces (see\cite{TODO} for
details) shared a common mode of interaction where the majority of
...
Through all of these performances every touch interaction---every
touch down, dragged and up---was timestamped and logged to disk for
later analysis.

These sessions were part of a long-term research project into music
touch-screen interaction, as well as an ongoing creative practice. The
log data collected over these sessions is homogeneous in form, but
represents a wide variety of performance contexts: rehearsals and
performances, written compositions and free improvisations, controlled
lab studies and casual demonstrations, with and without additional
acoustic instruments (some of these are shown in
Figure~\ref{fig:three-performance-contexts}).
...
are shown in Table \ref{corpus-table}. The session data includes
metadata regarding the session \emph{context} (whether it was a
performance, rehearsal, study, demonstration, or data collection
session), session \emph{type} (either improvisation or composition),
and the instruments used (iPads alone, or iPads with acoustic
percussion). The distribution of recorded sessions by context and type
is shown in Figure \ref{fig:count-data}, and we shall use these
groupings for our analysis in
Section~\ref{differentiating-interaction-sessions}.
...
In some of these sessions, surveys and open-ended interviews were
conducted with the participating musicians in an effort to understand
the interaction dynamics and felt experience of the participants.
However, many of these sessions were in-the-wild musical performances,
and in these cases no such subjective evaluation data was
collected---only the log data. This raises the question: can we use
the log data to fill in some of the missing ``subjective experience''
data, and to understand broader trends in the interaction behaviour of
the musicians over this period?
...
From previous work\cite{TODO} we have a robust technique for
segmenting the raw touch data generated by the instrument into a
sequence of high-level ``gestures'' using a Random
