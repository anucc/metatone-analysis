...
The matrix is normalised so that the sum of all elements will be one
...
sequence is defined as the ratio of changed transitions (e.g.
$A \rightarrow B$), to self transitions (e.g. $A \rightarrow A $). The
the flux of a transition matrix $P$ can easily be calculated:
\begin{equation}
  \mathrm{flux}(P) = 1 - \mathrm{tr}(P)
\end{equation}
Where the trace of P, $tr(P)$ is given by the sum of the diagonal
entries of $P$.
...
flux measure to analyse a corpus of data collected as part of our
research into collaborative interactions with musical touch-screen
tablet interfaces.
